...
public class RecommendationsTest {
    ...
    @Test
    public void normalSeveralDisjointRecommendationTest() {
        //расставляем лайки так, чтобы ближайших пользователей было несколько
        filmService.addLike(1L, 1L);
        filmService.addLike(2L, 1L);
        filmService.addLike(3L, 1L);
        ...
        filmService.addLike(3L, 4L);
        filmService.addLike(8L, 4L);
        //выбираем рекомендации всех ближайших пользователей (их трое: 2, 3 и 4)
        List<Film> films = recommendationService.recommendBySeveralUsers(1);
        for (Film film : films) {
            System.out.println(film);
        }
        //проверяем рекомендации
        assertEquals(films.size(), 3); //все рекомендации разные
        assertEquals(films.get(0).getId(), 6);
        assertEquals(films.get(1).getId(), 7);
        assertEquals(films.get(2).getId(), 8);
    }

    @Test
    public void normalSeveralRecommendationWithIntersectionsTest() {
        //расставляем лайки так, чтобы ближайших пользователей было несколько
        filmService.addLike(1L, 1L);
        filmService.addLike(2L, 1L);
        filmService.addLike(3L, 1L);
        ...
        //проверяем рекомендации
        assertEquals(films.size(), 2); //рекомендации пересекаются
        assertEquals(films.get(0).getId(), 6);
        assertEquals(films.get(1).getId(), 7);
    }
}
